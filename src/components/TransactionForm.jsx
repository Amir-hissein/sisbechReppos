    ...
    return (
        <div className="bg-white p-4 md:p-8 rounded-2xl shadow-soft border border-gray-100 relative overflow-hidden">
                ...
                <div className="flex bg-gray-100 p-1 md:p-1.5 rounded-xl border border-gray-200">
                    <button
                        ...
                        className={`flex-1 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-bold transition-all duration-300 flex justify-center items-center gap-1.5 ${feeCalculation === 'deduct'
                            ? 'bg-white text-teal-600 shadow-sm ring-1 ring-black/5 scale-100'
                            : 'text-text-muted hover:text-text-main hover:bg-gray-200/50 scale-95'
                            }`}
                    >
                        FRAIS INCLUS (- {formData.percentage || 0}%)
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        FRAIS EN SUS (+ {formData.percentage || 0}%)
                    </button>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 relative z-10">
                ...
                {/* Live Results Card */}
                <div className="bg-gradient-to-br from-teal-500/5 to-transparent p-4 md:p-6 rounded-2xl border border-teal-500/10 mt-6 md:mt-8 relative group overflow-hidden transition-all hover:shadow-md">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-teal-500/5 rounded-full blur-2xl -mr-10 -mt-10"></div>
                    ...
                    <div className="flex justify-between items-center text-xs md:text-sm mb-3 md:mb-4 relative z-10">
                        <span className="text-text-muted font-medium flex items-center gap-2">
                            {feeCalculation === 'deduct' ? 'Frais à déduire' : 'Frais à ajouter'} <ArrowRight className="w-3 h-3 text-teal-600" />
                        </span>
                        <span className="font-mono text-text-main font-bold bg-white/50 px-2 py-1 rounded-md text-sm md:text-base">
                            {feeCalculation === 'deduct' ? '-' : '+'} {formatMoney(fee, activeCurrency)}
                        </span>
                    </div>

                    <div className="flex justify-between items-end border-t border-teal-500/10 pt-3 md:pt-4 mt-2 relative z-10">
                        <span className="text-slate-700 font-bold text-base md:text-lg">
                            {feeCalculation === 'deduct' ? 'Montant Net' : 'Total à Payer'}
                        </span>
                        <div className="text-right">
                            <span className="font-mono text-teal-600 text-2xl md:text-3xl font-bold tracking-tight block">{formatMoney(total, activeCurrency)}</span>
